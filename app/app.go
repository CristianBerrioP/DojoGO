...
func create(w http.ResponseWriter, r *http.Request){
	...
	c := session.DB("dojogo").C("people")
	err = c.Insert(&Person{usuario.Name, usuario.Phone, usuario.Id, usuario.Email, usuario.Direction})
	if err != nil {
					log.Fatal(err)
	}
...
func getByName(w http.ResponseWriter, r *http.Request){
	...
	var usuarios []Person
	c := session.DB("dojogo").C("people")
	err = c.Find(bson.M{"Name": "dos"}).All(&usuarios)
	respuesta, err :=  json.Marshal(usuarios)
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
	w.Write(respuesta)
...
func getById(w http.ResponseWriter, r *http.Request){
	...
	var usuarios []Person
	c := session.DB("dojogo").C("people")
	err = c.Find(bson.M{"Id": id}).All(&usuarios)
	respuesta, err :=  json.Marshal(usuarios)
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
	w.Write(respuesta)
...
func main(){
	...
	r.HandleFunc("/getUserById/{userId}", getById).Methods("GET")
